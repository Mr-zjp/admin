...
<script>
  ...
  export default {
    ...
    mounted(){
        console.log(this.orgNoOptions);
        this.mechanismId=this.orgNoOptions.length===0?'':this.orgNoOptions[0].orgNo;
        this.mechanismName=this.orgNoOptions.length===0?'':this.orgNoOptions[0].orgName;
        this.getDataList()
    },
    methods: {
        // 机构树选中
        orgListTreeCurrentChangeHandle (data) {
            this.mechanismId = data.orgNo
            this.mechanismName = data.orgName
            console.log(data)
            this.pageIndex=1;
            this.pageSize=10;
            this.searchForm.userName='';
            this.getDataList(data.orgNo)
        },
      // 获取数据列表
      getDataList (orgNo) {
        this.$http({
